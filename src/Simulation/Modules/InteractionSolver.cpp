...
#include "Simulation/Modules/InteractionSolver.h"
...
void InteractionSolver::eachToEachSolve(LinearContainer<Boid> &container, Boid &target) {
    ...
    if (neighboringBoidsCnt) {
        target.m_acceleration += (neighboringVelocities / (float) neighboringBoidsCnt - target.m_velocity) *
                                 Settings::Model::MathingFactor;
        target.m_acceleration += (neighboringPositions / (float) neighboringBoidsCnt - target.m_position) *
                                 Settings::Model::CenteringFactor;
    }
